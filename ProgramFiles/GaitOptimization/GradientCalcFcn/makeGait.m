...
function gait = makeGait(y,t0,invert)
    ndim = size(y,2);
    ...
    if invert == 0
        for dim = 1:ndim
            gait.phi_def{dim} = @(t) phi_def(t+t0,y(:,dim));
            gait.dphi_def{dim} = @(t) dphi_def(t+t0,y(:,dim));
            gait.ddphi_def{dim} = @(t) ddphi_def(t+t0,y(:,dim));
        end
    else
        for dim = 1:ndim
            gait.phi_def{dim} = @(t) phi_def(-t+t0,y(:,dim));

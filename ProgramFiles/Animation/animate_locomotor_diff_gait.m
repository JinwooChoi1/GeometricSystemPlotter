function animate_locomotor_diff_gait(export,info_needed)
...
%         'renderer','painters');
    ...
    shchname = cell(n_diff_gaits,1);


    % Forward : opt_9d08b2181adcfbf2bbe1f1cd3d8209f0
    % Lateral : opt_b87a16ad2073de134fa556eb89229268
    % Turning : opt_6519c68f4c4a00701de609df7b9f6f51

	shchname{1} = 'opt_gait1';
    shchname{2} = 'opt_gait2';
	shchname{3} = 'opt_gait1';
    shchname{4} = 'opt_gait2';
    
    shaperaw = cell(n_diff_gaits,1);
    posraw = cell(n_diff_gaits,1);

    for i = 1:length(shchname)
        gaitfile = fullfile(data_source,['sysf_' system_name '__shchf_' shchname{i}]);
        load(gaitfile);
	    shaperaw{i} = p.phi_locus_full{1}.shape;
    
        if strcmp(info_needed.Coordinates,'minperturbation_coords')
            posraw{i} = p.G_locus_full{1}.G_opt;
            h.drawing_baseframe = info_needed.current_system2;
        else
            posraw{i} = p.G_locus_full{1}.G;
        end
	...
	n_gaits = 2*ones(1,n_diff_gaits);
	%start_pos = -(n_gaits(1)/2);
	
	% get full configuration history
	[h.shapedata, h.posdata] = gait_concatenator(shaperaw,posraw,n_gaits);
...
%%%%%%
function [shapedata, posdata] = gait_concatenator(shaperaw,posraw,n_gaits)

    % Initialize the position data with an empty matrix
    posdata = [];
    shapedata = [];
    cyclic_displacement = cell(length(shaperaw),1);
    cyclic_displacement_m = cell(length(shaperaw),1);
    if length(shaperaw) == length(n_gaits)
        for i = 1:length(shaperaw)
            % Store the shape and data to the frameinfo structure
            shapedata = cat(1,shapedata,shaperaw{i}(1,:),repmat(shaperaw{i}(2:end,:),[n_gaits(i),1]));

            %%%%%%
            % Concatenate the gait displacements together.

            % Get the displacement over one cycle
            cyclic_displacement{i} = posraw{i}(end,:);

            % convert this cyclic displacement into an SE(2) matrix
            cyclic_displacement_m{i} = vec_to_mat_SE2(cyclic_displacement{i});
            
            % Add a set of ones onto the xy values, so that they can be
            % translated/rotated by SE(2) matrices
            posraw_xy_augmented = cat(1, posraw{i}(:,1:2)', ones(1,size(posraw{i},1)));
            ...
            for j = 1:n_gaits(i)
                ...
                previous_displacement_m = eye(3,3);
                if i > 1
                    for k = 1:i-1
                    previous_displacement_m = previous_displacement_m*... 
                        powerm_pade(cyclic_displacement_m{k},n_gaits(k));
                    end
                end
                previous_displacement_m = previous_displacement_m*... 
                    powerm_pade(cyclic_displacement_m{i},(j-1));

                posdata_xy_augmented = ...
                    previous_displacement_m ...
                ...
                previous_displacement = mat_to_vec_SE2(previous_displacement_m);
                
                % theta component (always starts at zero orientation)
                posdata_theta = ...
                    posraw{i}(first_index:end,3)'... % Take the theta values from the within-gait displacement
                    + previous_displacement(3); % Add as many net theta changes as there are previous gait cycles
        ...
        error('The size of n_gaits is different from the size of shaperaws');
    end
    shapedata(:,1) = lowpass(shapedata(:,1),1e2,1e3);
    shapedata(:,2) = lowpass(shapedata(:,2),1e2,1e3);
end
